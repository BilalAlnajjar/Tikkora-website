...
@section('content')
    ...
    {{--    <script async type="text/javascript" src="https://unpkg.com/@eyevinn/web-player-component@0.8.4/dist/web-player.component.js"></script>--}}

    @if($section_one)
        <div class="section">
            {!!html_entity_decode($section_one->item)!!}
        </div>
    @endif
    {{--    <div class="px-80 py-4">--}}
    {{--        <eyevinn-video source="{{$link}}" ></eyevinn-video>--}}
    {{--    </div>--}}
    <div class="flex flex-col items-center">
        <div class="flex flex-row-reverse gap-3">
            @foreach($links as $link)
                <button type="button"
                        class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-1 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-1.5 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"
                        onclick='getLink("{{$link->url}}")'>{{$link->title}}</button>
            @endforeach
        </div>
        <div id='{{$string_id}}' class="flex flex-row justify-center w-1/2 h-96 my-4">
            {{--            <iframe width="100%" height="100%" src="https://ntuplay.xyz/embed.php?id=HTcrSpzalK"--}}
            {{--                    sandbox="allow-same-origin allow-scripts" scrolling="no" frameborder="0"--}}
    ...
    @if($section_two)
        <div class="section">
            {!!html_entity_decode($section_two->item)!!}
        </div>
    @endif
    ...
    {{--    <script>--}}
    {{--        var player = videojs('{{$string_id}}');--}}
    {{--        player.play();--}}
    {{--    </script>--}}

    <script>
        var addToMeElement = $("#{{$string_id}}");
        $(document).ready(function () {
            addToMeElement.html('<iframe width="100%" height="100%"  src="{{$links->first()->url}}"'+
                'sandbox="allow-same-origin allow-scripts" scrolling="no" frameborder="0"'+
                'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"'+
                'allowfullscreen=""></iframe>')
        });
        ...
        function getLink(url) {
            addToMeElement.html('<iframe width="100%" height="100%"  src="' + url + '">' +
                'sandbox="allow-same-origin allow-scripts" scrolling="no" frameborder="0"' +
                'allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"' +
                'allowfullscreen=""></iframe>')
        }
